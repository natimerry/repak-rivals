...
#pragma warning restore CS0168 // Variable is declared but never used
        ...
        /// <summary>
        /// Initializes all internal fields required for Write() to work on a programmatically-created asset.
        /// Call this method after setting up Imports, Exports, and the name map to prepare for serialization.
        /// </summary>
        public void InitializeForWriting()
        {
            // Initialize collections that Write() expects
            if (Generations == null || Generations.Count == 0)
            {
                Generations = new List<FGenerationInfo> { new FGenerationInfo(Exports?.Count ?? 0, GetNameMapIndexList()?.Count ?? 0) };
            }
            
            if (AdditionalPackagesToCook == null)
            {
                AdditionalPackagesToCook = new List<FString>();
            }
            ...
            // Initialize AssetRegistryData
            if (AssetRegistryData == null)
            {
                AssetRegistryData = Array.Empty<byte>();
            }
            
            // Initialize BulkData if needed
            if (BulkData == null)
            {
                BulkData = Array.Empty<byte>();
            }
            
            // Disable optional data sections that aren't needed
            doWeHaveAssetRegistryData = AssetRegistryData.Length > 0;
            doWeHaveWorldTileInfo = false;
            
            // Set version-related fields for UE5 unversioned
            if (LegacyFileVersion == 0)
            {
                LegacyFileVersion = -8; // UE5
            }
            
            // Initialize engine versions if not set (check if Major is 0 as a proxy for uninitialized)
            if (RecordedEngineVersion.Major == 0 && RecordedEngineVersion.Minor == 0)
            {
                RecordedEngineVersion = new FEngineVersion(5, 3, 2, 0, new FString("++UE5+Release-5.3"));
            }
            
            if (RecordedCompatibleWithEngineVersion.Major == 0 && RecordedCompatibleWithEngineVersion.Minor == 0)
            {
                RecordedCompatibleWithEngineVersion = new FEngineVersion(5, 3, 2, 0, new FString("++UE5+Release-5.3"));
            }
            
            // Set default flags
            if (PackageFlags == 0)
            {
                PackageFlags = EPackageFlags.PKG_FilterEditorOnly | EPackageFlags.PKG_Cooked;
            }
